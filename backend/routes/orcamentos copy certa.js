  ...
  return new Promise((resolve, reject) => {
    ...
    db.get(
      ...
      (err, unitario) => {
        ...
        if (unitario) {
          const valorProduto = Number(unitario.preco);
        ...
        /* =====================================================
           🔵 2️⃣ PRODUTO POR VÃO (LÓGICA ATUAL — MANTIDA)
           ===================================================== */
        ...
        if (!largura || !altura) {
          return reject(
            new Error('Largura e altura são obrigatórias para produtos por vão')
          );
        }

        // 👉 SE VEIO PREÇO POR M² DO FRONTEND
        if (item.preco_m2) {
        ...
        db.get(
          ...
          WHERE produto_id = ?
            AND largura_max >= ?
            AND altura_max >= ?
          ORDER BY largura_max ASC, altura_max ASC
          LIMIT 1
          `,
          [item.produto_id, largura, altura],
          (err, row) => {
            if (err || !row) {
              return reject(
                new Error('Preço não encontrado para este vão')
              );
            }

            const valorProduto = Number(row.preco);
            const totalItem = valorProduto;
...
router.get('/:id/pdf', (req, res) => {
  ...
  db.get(`
    ...
    WHERE o.id = ?
  `, [id], (err, orc) => {
    if (err || !orc) return res.status(404).send('Não encontrado');
    ...
    `, [id], (err, itens) => {
      ...
      doc.text(`Validade do orçamento: ${formatarData(dataValidade)}`);
      doc.moveDown();
      ...
      itens.forEach(i => {
        ...
        doc
          .fontSize(11)
          .text(`${i.nome}${medidas}`, { continued: true })
      ...
      doc
        ...
        .fillColor('#555')
        .text(
          'Este orçamento é válido por 10 (dez) dias a partir da data de emissão. ' +
          'Após esse prazo, valores e condições poderão sofrer alterações.'
        );
